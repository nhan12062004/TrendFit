...
// --- Constants ---
const DAYS = [
  { id: 0, label_vi: 'T2', full_vi: 'Thứ 2' },
  { id: 1, label_vi: 'T3', full_vi: 'Thứ 3' },
  { id: 2, label_vi: 'T4', full_vi: 'Thứ 4' },
  { id: 3, label_vi: 'T5', full_vi: 'Thứ 5' },
  { id: 4, label_vi: 'T6', full_vi: 'Thứ 6' },
  { id: 5, label_vi: 'T7', full_vi: 'Thứ 7' },
  { id: 6, label_vi: 'CN', full_vi: 'Chủ Nhật' },
];

const MEAL_TYPES = [
  { id: 'breakfast', label_vi: 'Bữa sáng', icon: Coffee, color: '#f59e0b', bgLight: 'rgba(245,158,11,0.08)' },
  { id: 'lunch', label_vi: 'Bữa trưa', icon: Sun, color: '#22c55e', bgLight: 'rgba(34,197,94,0.08)' },
  { id: 'dinner', label_vi: 'Bữa tối', icon: MoonIcon, color: '#818cf8', bgLight: 'rgba(129,140,248,0.08)' },
  { id: 'snack', label_vi: 'Bữa phụ', icon: Cookie, color: '#fb923c', bgLight: 'rgba(251,146,60,0.08)' },
];

const FOOD_CATEGORIES = ['Chất đạm', 'Carbs', 'Rau củ', 'Trái cây', 'Sữa', 'Bữa phụ'];
...
// --- Add New Food Modal ---
const AddNewFoodModal = ({ isOpen, onClose, onCreated }: {
  isOpen: boolean; onClose: () => void; onCreated: (food: Food) => void;
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md bg-bg-secondary border border-border-primary rounded-2xl p-5 mx-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-black text-text-primary">Thêm món mới</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[10px] font-bold text-text-tertiary uppercase tracking-wider">Tên món</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-text-tertiary uppercase tracking-wider">Danh mục</label>
            ...
            <div>
              <label className="text-[10px] font-bold text-text-tertiary uppercase tracking-wider">Đơn vị</label>
...
// --- Meal Section ---
const MealSection = ({ mealType, items, onAdd, onDelete, onUpdateQty }: {
  mealType: typeof MEAL_TYPES[0]; items: PlanFood[];
    ...
    <div ref={setNodeRef} className={`bg-bg-secondary border rounded-2xl overflow-hidden transition-all duration-200 ${isOver ? 'border-[#a3e635] ring-1 ring-inset ring-[#a3e635] bg-[#a3e635]/10' : 'border-border-primary'}`}>
      <div className="px-3 sm:px-4 py-2.5 flex items-center justify-between" style={{ borderBottom: items.length > 0 ? '1px solid var(--border-primary)' : 'none' }}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs sm:text-sm font-bold text-text-primary">{mealType.label_vi}</h4>
            {items.length > 0 && <span className="text-[9px] text-text-tertiary">{items.length} món • {totalCal} kcal</span>}
          </div>
        </div>
...
const [foods, setFoods] = useState<Food[]>([]);
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={pointerWithin} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => setActiveDrag(null)}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <input type="text" value={planName} onChange={e => setPlanName(e.target.value)}
                className="text-lg sm:text-xl font-black text-text-primary bg-transparent border-none outline-none focus:ring-0 p-0 w-full sm:w-auto" placeholder="Kế hoạch ăn uống" />
            </div>
              ...
              <div className="bg-bg-secondary border border-border-primary rounded-2xl p-3 sm:p-4 shrink-0">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-[9px] font-black uppercase tracking-widest text-text-tertiary">
                    Dinh dưỡng — {DAYS[selectedDayId].full_vi}
                  </h3>
                </div>
                ...
                <div className="space-y-2.5">
                  {MEAL_TYPES.map(meal => (
                    <MealSection key={meal.id} mealType={meal}
                      items={dayItems.filter(i => i.meal_type === meal.id)}
      ...
      {/* Add New Food Modal */}
      <AddNewFoodModal isOpen={addFoodModalOpen} onClose={() => setAddFoodModalOpen(false)} onCreated={handleNewFoodCreated} />
